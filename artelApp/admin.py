...
class categoryAdmin(admin.ModelAdmin):
    ...
    list_display = ("category_name_uz", "category_name_ru", "category_name_us",
                    "category_name_tr", 'image_category', 'counter_total_product', 'artel_or_texno_park')
    list_filter = (
...
class goodsAdmin(admin.ModelAdmin):
    ...
    def get_images(self, obj=None):
        find = good_images.objects.filter(
            good_id=obj).exclude(good_img='').count()
        if find >= 1:
            preview = [
    ...
    def get_badge(self, obj=None):
        find = good_images.objects.filter(
            good_id=obj).exclude(good_badge='').count()
        if find >= 1:
            preview = [
                s.good_badge.url for s in good_images.objects.filter(good_id=obj).exclude(good_badge='')]
            return mark_safe('<img src="%s" style="width: 60px; height:60px;" />' % preview[0])
        else:
            return 'Нет бадже'
...
class companyAdmin(admin.ModelAdmin):
    fields = (("company_icon", 'image_icon'), 'company_short_info_uz', 'company_short_info_ru', 'company_short_info_us',
              'company_short_info_tr', 'company_info_uz', 'company_info_ru', 'company_info_us', 'company_info_tr', )
    list_display = ("company_short_info_uz", "company_short_info_ru",
                    "company_info_uz", "company_info_ru", 'image_icon')
...
class ecologAdmin(admin.ModelAdmin):
    fields = ('ecology_desc_uz', 'ecology_desc_ru', 'ecology_desc_us',
              'ecology_desc_tr', ("ecology_img", 'image_ecology'))
...
class innovationAdmin(admin.ModelAdmin):
    fields = ('innovation_desc_uz', 'innovation_desc_ru', 'innovation_desc_us',
              'innovation_desc_tr', ('innovation_img', 'image_innovation'))
    list_display = ('innovation_desc_uz',
                    'innovation_desc_ru', 'image_innovation')
...
class productBaseAdmin(admin.ModelAdmin):
    fields = ('product_bases_desc_uz', 'product_bases_desc_ru', 'product_bases_desc_us',
              'product_bases_desc_tr', ('product_bases_img', 'image_prod_base'))
    list_display = ('product_bases_desc_uz',
                    'product_bases_desc_ru',  'image_prod_base')
...
class stockAdmin(admin.ModelAdmin):
    fields = ("title_uz", "title_ru", "title_us", "title_tr", ("icon", 'icon_stock'), ("banner", 'banner_stock'),
              "description_uz", "description_ru", "description_us", "description_tr", "start_date", "end_date")
    list_display = ('title_uz', 'title_ru', "title_us", "title_tr",
                    'icon_stock', 'banner_stock', 'start_date', 'end_date')
...
class servic(admin.ModelAdmin):
    list_display = ('service_city_uz', 'service_city_ru',
                    'service_address_list_uz', 'service_address_list_ru', 'service_phone', )
    ('service_city_uz', 'service_city_ru', 'service_city_us',
     'service_city_tr', 'service_address_list_uz',)
